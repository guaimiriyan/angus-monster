...
import selfMvc.mvcframework.annotation.GPRequestParam;
 ...
 */
public class AngusHandlerAdapter {
    public boolean supports(Object handler){ return (handler instanceof AngusHandlerMapping);}

//ModelAndView
     Object handle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
        AngusHandlerMapping handlerMapping = (AngusHandlerMapping)handler;
        ...
        boolean isModelAndView = handlerMapping.getMethod().getReturnType() == Object.class;//GPModelAndView.class;
        if(isModelAndView){
            return result;//(GPModelAndView)
        }
        return null;
    }
